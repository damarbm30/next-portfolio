        ...
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            ...
            >
              ...
              >
                {link.name}
                {/* inset-0 means top, left, right, and bottom equals 0 */}
                {activeSection === link.name && (
                  <motion.span
                    layoutId="activeSection"
                    transition={{
                      type: "spring",
                      // force given to move the spring
                      stiffness: 400,
                      // how much "force" to give to the spring animation, more damp = less bouncy & speed
                      damping: 25,
                    }}
